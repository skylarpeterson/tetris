...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for(int i = 0; i < this.width; i++) this.skirt[i] = -1;
		for(int n = 0; n < points.length; n++){
			int x = points[n].x;
			int y = points[n].y;
			if(this.skirt[x] > y || this.skirt[x] == -1) this.skirt[x] = y;
		}
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for(int i = 0; i < this.body.length; i++){
			int newX = this.height - this.body[i].y - 1;
			int newY = this.body[i].x;
	...
	*/	
	public Piece fastRotation() {
		if(next == null) Piece.makeFastRotations(this);
		return next;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		for(int i = 0; i < objBody.length; i++){
			points.add(objBody[i]);
		}
		for(int n = 0; n < this.body.length; n++){
			if(!points.contains(this.body[n])) return false;
		}
		return true;
	}
